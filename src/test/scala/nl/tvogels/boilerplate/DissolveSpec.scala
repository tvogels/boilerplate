...
class OracleSpec extends UnitSpec {
  ...
  val NUM_WEIGHT_VECS = 5000 // = # times each test case is attempted
  ...
  "F(x_m, y*)" should "be >= F(x_m, y_m)" in {
    ...
    assert(negDiffSeq.length == 0,
      "%d / %d cases failed (min %e)".format(negDiffSeq.length, diffSeq.length, diffSeq.min))
